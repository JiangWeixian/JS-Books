...
写`JavaScript`代码的时候会发现，其实有些代码是同步执行的，并不会等一个代码结束之后，立刻执行另外一个代码。有可能是几块代码同步执行！

不过这一点是由于几块不同的代码分享的是同一个作用域，对状态修改的同步执行。导致出现以上情况。

这一章好难理解。最大的感觉就是`JavaScript`由于一同执行可能导致很多问题。

这种情况当和服务器互相交互的时候特别特别明显。**因为不太确定返回的数据不一定按照请求的先后顺序到达。**
